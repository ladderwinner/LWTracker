...
/// The LW Tracker is a Swift framework to send analytics to the LW server.
///
/// ## Basic Usage
/// * Use the track methods to track your views, events and more.
final public class LWTracker: NSObject {
    ...
    @objc public var isOptedOut: Bool {
        get {
            return lwUserDefaults.optOut
        }
        set {
            lwUserDefaults.optOut = newValue
        }
    }
    ...
    @objc public var userId: String? {
        get {
            return lwUserDefaults.visitorUserId
        }
        set {
            lwUserDefaults.visitorUserId = newValue
            visitor = Visitor.current(in: lwUserDefaults)
        }
    }
    ...
    @objc public var forcedVisitorId: String? {
        get {
            return lwUserDefaults.forcedVisitorId
        }
        set {
            ...
            if let newValue = newValue {
                ...
                if isValidString {
                    lwUserDefaults.forcedVisitorId = newValue
                } else {
                ...
                lwUserDefaults.forcedVisitorId = nil
            }
            visitor = Visitor.current(in: lwUserDefaults)
        }
    }
    
    internal var lwUserDefaults: LWUserDefaults
    ...
    /// This logger is used to perform logging of all sorts of LW related information.
    ...
    internal static var _sharedInstance: LWTracker?
    ...
    required public init(siteId: String, queue: Queue, dispatcher: Dispatcher) {
        ...
        self.lwUserDefaults = LWUserDefaults(suiteName: "\(siteId)\(dispatcher.baseURL.absoluteString)")
        self.visitor = Visitor.current(in: lwUserDefaults)
        self.session = Session.current(in: lwUserDefaults)
        super.init()
        startNewSession()
        startDispatchTimer()
    }
    ...
    /// - Parameters:
    ///   - siteId: The unique site id generated by the server when a new site was created.
    ///   - baseURL: The url of the LW server. This url has to end in `LW.php` or `lw.php`.
    ...
    @objc convenience public init(siteId: String, baseURL: URL, userAgent: String? = nil) {
        let validSuffix = baseURL.absoluteString.hasSuffix("tracker.php")
        assert(validSuffix, "The baseURL is expected to end in tracker.php")
    ...
    @objc public func dispatch() {
        guard !isDispatching else {
            logger.verbose("LWTracker is already dispatching.")
            return
        }
    ...
    ///
    /// More information on campaigns: [https://lw.org/docs/tracking-campaigns/](https://lw.org/docs/tracking-campaigns/)
    ///
    /// - Parameters:
    ...
    /// More information on content: [https://lw.org/docs/content-tracking/](https://lw.org/docs/content-tracking/)
    ///
    /// - Parameters:
...
extension LWTracker {
    ...
    public func startNewSession() {
        lwUserDefaults.previousVisit = lwUserDefaults.currentVisit
        lwUserDefaults.currentVisit = Date()
        lwUserDefaults.totalNumberOfVisits += 1
...
extension LWTracker {
    ...
    /// Tracks a screenview.
    ///
    /// This method can be used to track hierarchical screen names, e.g. screen/settings/register. Use this to create a hierarchical and logical grouping of screen views in the LW web interface.
    ///
    ...
    /// Tracks an event as described here: https://lw.org/docs/event-tracking/
    ///
    /// - Parameters:
    ...
    /// Tracks a goal as described here: https://lw.org/docs/tracking-goals-web-analytics/
    ///
    /// - Parameters:
    ...
    /// Tracks an order as described here: https://lw.org/docs/ecommerce-analytics/#tracking-ecommerce-orders-items-purchased-required
    ///
    /// - Parameters:
    ...
    public func trackOrder(id: String, items: [OrderItem], revenue: Float, subTotal: Float? = nil, tax: Float? = nil, shippingCost: Float? = nil, discount: Float? = nil) {
        let lastOrderDate = lwUserDefaults.lastOrder
        ...
        lwUserDefaults.lastOrder = Date()
    }
}

extension LWTracker {
    
    /// Tracks a search result page as described here: https://lw.org/docs/site-search/
    ///
    /// - Parameters:
...
extension LWTracker {
    ...
    /// - Parameter index: The index of the dimension. A dimension with this index must be setup in the LW backend.
...
extension LWTracker {
    @objc public func copyFromOldSharedInstance() {
        lwUserDefaults.copy(from: UserDefaults.standard)
    }
}
